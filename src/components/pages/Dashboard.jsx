...
const activeClients = clients.filter(client => (client.status_c || client.status) === "Active").length;
      ...
      const tasksDueToday = tasks.filter(task => 
        !(task.completed_c || task.completed) && (task.dueDate_c || task.dueDate) === today
      ).length;
      
      const overdueTasks = tasks.filter(task => {
        if ((task.completed_c || task.completed) || !(task.dueDate_c || task.dueDate)) return false;
        return new Date(task.dueDate_c || task.dueDate) < new Date(today);
      }).length;
